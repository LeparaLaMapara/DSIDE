...
def guarded(name: str, fetch: Callable[[], pd.DataFrame]) -> pd.DataFrame:
    ...
    try:
        ...
        state[name] = {"ok": True, "fetched_at": now}
        ...
        state[name] = {"ok": False, "fetched_at": last, "failed_at": now, "error": f"{type(exc).__name__}: {exc}"[:200]}
