...
import { contains, locate, type LngLat } from "@/lib/geo";
...
export function WardMap({ code, wardYearNote }: { code: string; wardYearNote: string }) {
  ...
  useEffect(() => {
    ...
    m.on("style.load", () => {
      ...
      m.addLayer({ id: "me-dot", type: "circle", source: "me",
  ...
  useEffect(() => {
    ...
    (m.getSource("me") as GeoJSONSource).setData(me ? { type: "FeatureCollection", features: [
      { type: "Feature", properties: {}, geometry: { type: "Point", coordinates: me } }] } : EMPTY);
  }, [ready, me, base]);
  ...
  const findMe = async () => {
    ...
    try {
      ...
      setLocMsg("");
      // Show the whole ward the dot falls in.
      ...
      walk((hit.geometry as GeoJSON.Polygon).coordinates);
      map.current?.fitBounds(box as [number, number, number, number], { padding: 24 });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (new URLSearchParams(window.location.search).get("locate") === "1") {
      ...
      // Start on the next tick, outside the effect body, so state is not set during it.
      // No cleanup: this runs once, and a re-render must not cancel it.
      setTimeout(() => void findMe(), 0);
    }
  });
